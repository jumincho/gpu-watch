...
- **エージェント不要。** 各 GPU サーバーに必要なのは SSH アクセス、`nvidia-smi`、`python3` だけです。サーバーに何かをインストールしたり常駐させたりしません。
...
- 幅 320px の画面まで対応し、キーボードだけで操作でき、OS の「動きを減らす」設定にも従います。配色は WCAG 2.2 AA のコントラスト基準を満たしています。
...
2. プロセスの所有者は `/proc` の UID と NSS 名から決め、PID の開始時刻と GPU UUID で再確認します。確認できない場合は推測せず、不明として表示します。
3. ディスク使用量は `disk_poll_interval_seconds`（30 分）ごとに、`df`、時間制限付きの `du`、`docker ps --size` で測定します。
...
- SSH 接続か GPU の取得かを問わず、GPU を完全に観測できなかったサーバーは **DOWN** です。その GPU はどれも利用可能に数えず、以前の測定値を現在の値として表示しません。

## セキュリティモデル

- **アクセス制御。** Caddy が IP 許可リストを適用します。アプリもクライアント IP、`Host`、`Origin` を独自に再確認します。
- **書き込み。** お知らせやタイマーの変更には、同一オリジンからのリクエストとパスフレーズまたは PIN が必要です。ハッシュには 600,000 回反復の PBKDF2-SHA256 を使います。失敗した試行はサブネット単位と全体の両方でレート制限します。
- **ブラウザーに送らないもの。** リモートコマンド、stderr、完全なコマンドライン、環境変数、SSH のユーザー、ポート、パスワードです。サーバーカードには検証済みの IP アドレスだけを表示します。
...
| 各 GPU サーバー | 監視用アカウントの SSH アクセス、`nvidia-smi` を含む NVIDIA ドライバー、`python3`、`df`、`du`。Docker は任意で、あればコンテナごとの使用量も表示します。 |
| 閲覧者 | 最新の Web ブラウザー |
...
このほかのトップレベルの設定には、プローブのタイムアウト、`collector_workers`、🔥 と ❄️ のバッジの基準を決める `activity_policy`、保存期間などがあります。既定では、イベントは 180 日、お知らせは 90 日、日次バックアップは 14 日間保存します。
...
Artificial Analysis のパネルを使うには、API キーを `secrets/artificial_analysis_api_key` に置いてください。シンボリックリンクではない通常のファイルで、パーミッションは `0600` にする必要があります。サーバーは指数を 6 時間ごとに更新し、失敗した場合は 1 時間ごとに再試行します。48 時間を超えたデータは古いデータとして表示します。
...
- `deploy.sh` は研究室の本番ホスト向けのデプロイスクリプトです。パスと権限を確認したあと、チェックサム付きの SQLite オンラインバックアップを作成します。続いて二つのイメージをビルドし、候補コンテナを検証します（health、snapshot、collector）。以前のコンテナを残したまま本番を切り替え、許可リストと状態を確認し、失敗すれば自動でロールバックします。ほかの環境で使う場合は、まずホスト固有のパスとアドレスを変更してください。

### Windows の緊急フォールバック

`emergency-local-fallback.ps1 -Action Status|Start|Stop` は、本番が停止しているあいだだけローカルのコピーを実行します。本番に接続できる場合、`-Force` を付けない限り `Start` は拒否されます。さらに `VERSION` とリリースのフィンガープリントが一致し、最新の収集サイクルが確認できる必要があります。ファイアウォールは研究室の LAN に対してだけ開きます。`Stop` はリスナー、ファイアウォールのルール、一時的なパスワードファイルを削除します。
...
# ローカルのバックアップから復元する（復元先とチェックサムを検証）
...
| `gpu_watch/` | 認証、プロセスの帰属判定、セキュリティ補助、履歴、Artificial Analysis クライアント |
| `static/` | フロントエンド（`index.html`、`app.js`、`styles.css`）と同梱のアイコン・国旗 |
| `hosts.json` | サーバー、研究室、収集の設定 |
...
| `scripts/` | ヘルスチェック、データ監査、バックアップと復元、管理者 PIN、SSH の補助 |
