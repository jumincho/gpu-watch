...
- 空き、使用可能、使用中、予約済みの容量。警告バッジは Disk タブの `Used` と同じサーバー全体の集計値（使用量の合計 /（使用量の合計 + 空き容量の合計））を使い、表示使用率が 90% 以上で警告します。同じファイルシステムの重複マウントは一度だけ数えます。
...
v3、2026-09-25 リリース。画面のフッターには `GPT-6 Astra Max (Implementation) · Claude Opus 5.5 Max (Frontend)` と表示されます。
