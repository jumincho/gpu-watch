...
- **画面の調整。** タイマーの残り時間がカードの縦方向の中央に来るようになり、長いタイマー名も文字を優先して表示します。スマートフォンでは Intelligence Index がより詰まったレイアウトになり、韓国語は語の区切りで改行します。
...
インストール後、そのホストに `"privileged_disk_helper": true` を設定します。ヘルパーがない場合は、同じ時間の枠内で通常権限の取得に切り替え、ユーザー別の使用量を一部の集計として表示します。
...
`emergency-local-fallback.ps1 -Action Status|Start|Stop` は、本番が停止しているあいだだけローカルのコピーを実行します。本番に接続できる場合、`-Force` を付けない限り `Start` は拒否されます。さらに `VERSION` とリリースのフィンガープリントが一致し、最新の収集サイクルが確認できる必要があります。ファイアウォールは研究室の LAN に対してだけ開きます。`Stop` はリスナー、ファイアウォールのルール、一時的なパスワードファイルを削除します。ローカルのデータベースは別なので、障害中に作成したお知らせやタイマーは、本番に戻す前に突き合わせてください。
...
v3、2026-09-25 リリース。画面のフッターには `GPT-6 Astra Max / Claude Opus 5.5 Max` と表示されます。
